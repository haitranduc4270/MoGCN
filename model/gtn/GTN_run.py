...
# from GTN_model import GCN
from gtn_model import GTN
...
def train(epoch, optimizer, features, edge_index, edge_attr, labels, idx_train):
    '''
    ...
    # output = GTN_model(features, edge_index, edge_attr)
    output = GTN_model(features, edge_index)
...
def test(features, edge_index, edge_attr, labels, idx_test):
    '''
    :param features: the omics features
    :param adj: the laplace adjacency matrix
    :param labels: sample labels
...
if __name__ == '__main__':
    ...
    print(adj)
    nonzero_idx = torch.nonzero(adj, as_tuple=False)
    
    # Extract source and target nodes for edge_index
    edge_index = nonzero_idx.t()  # Shape: [2, num_edges]
    
    # Extract edge weights for edge_attr
    edge_attr = adj[nonzero_idx[:, 0], nonzero_idx[:, 1]]  # Shape: [num_edges]
    ...
    if args.mode == 0:
        ...
        for idx_train, idx_test in skf.split(data.iloc[:, 1:], label.iloc[:, 1]):
            # initialize a model
            # GTN_model = GCN(n_in=features.shape[1], n_hid=args.hidden, n_out=args.nclass, dropout=args.dropout)
            GTN_model = GTN(num_features=features.shape[1], num_classes=args.nclass, hidden_feats=args.hidden, num_layers=2, dropout=args.dropout)
            print(f'Total parameters: {sum(p.numel() for p in GTN_model.parameters())}')
            
            GTN_model.to(device)

            # define the optimizer
            optimizer = torch.optim.Adam(GTN_model.parameters(), lr=args.learningrate, weight_decay=args.weight_decay)
            ...
            for epoch in range(args.epochs):
                train(epoch, optimizer, features, edge_index, edge_attr, labels, idx_train)

            # calculate the accuracy and f1 score
            ac, f1= test(features, edge_index, edge_attr, labels, idx_test)
            acc_res.append(ac)
            f1_res.append(f1)
        ...
        for epoch in range(args.epochs):
            loss_values.append(train(epoch, optimizer, features, edge_index, edge_attr, labels, idx_train))
            ...
            torch.save(GTN_model.state_dict(), 'model/GTN/{}.pkl'.format(epoch))

            #reserve the best model, delete other models
            files = glob.glob('model/GTN/*.pkl')
            for file in files:
                name = file.split('/')[-1]
                print(file, name)
        ...
        GTN_model.load_state_dict(torch.load('model/GTN/{}.pkl'.format(best_epoch)))
